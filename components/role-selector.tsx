...
const RoleSelector = ({ onChange }: Props) => {
  return (
    <div>
      <Label className="mb-2" htmlFor="role">
        Select your role
      </Label>
      <RadioGroup className="gap-2 max-w-[400px]" onValueChange={onChange}>
        {/* Radio card #1 */}
        <div className="relative flex w-full items-start gap-2 rounded-lg border border-input p-4 shadow-sm shadow-black/5 has-[[data-state=checked]]:border-ring">
          <RadioGroupItem
            value="user"
            id={`user-toggle`}
            aria-describedby={`user-toggle`}
          ...
          <div className="flex grow items-start gap-3">
            ...
            <div className="grid grow gap-2">
              ...
              <p id={`user-tggle`} className="text-xs text-muted-foreground">
                Find and book available parking spots and EV charging stations
        ...
        <div className="relative flex w-full items-start gap-2 rounded-lg border border-input p-4 shadow-sm shadow-black/5 has-[[data-state=checked]]:border-ring">
          ...
          <div className="flex grow items-start gap-3">
            ...
            <div className="grid grow gap-2">
              <Label htmlFor={`host-toggle`}>
                Host{" "}
                <span className="text-xs font-normal leading-[inherit] text-muted-foreground">
                  (Sublabel)
                </span>
              </Label>
              <p id={`host-toggle`} className="text-xs text-muted-foreground">
                Share your unused parking spaces or EV chargers and earn passive
                income.
              </p>
            </div>
          </div>
