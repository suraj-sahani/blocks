...
import { ADD_EV_SCHEMA } from "@/lib/schema";
import { AddEVSchema, City, State } from "@/lib/types";
...
import { addEVStation, addEVStationImages } from "@/lib/action/location.action";
type Props = {
  states: State[];
};

export default function AddEVStationForm({ states }: Props) {
  const form = useForm({
    defaultValues: {
      name: "",
      address: "",
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (schema: AddEVSchema) => {
      const {
        success: isEVStationSuccess,
        data: evStation,
        message: evStationMessage,
      } = await addEVStation(schema);

      if (!isEVStationSuccess || !evStation) {
        return {
          success: false,
          message: evStationMessage,
        };
      ...
      if (Array.isArray(schema.images) && schema.images.length > 0) {
        ...
        } = await uploadImageToImageKit(schema.images, IMAGEKIT_FOLDERS.ev);
        ...
        const {
          success: isEVStationImageSuccess,
          message: evStationImageMessage,
          data: evStationImages,
        } = await addEVStationImages(
          JSON.stringify(uploadedImages),
          evStation.id
        );

        if (!isEVStationImageSuccess || !evStationImages) {
          return {
            success: false,
            message: evStationImageMessage,
          };
